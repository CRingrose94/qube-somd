...
if not os.path.exists(restart_file.val):
    ...
    if os.path.exists(s3file.val) :
        ...
        print("reading x3file")
    elif from_QuBe.val == True:
        (molecules, space) = assignVirtualSites(pdbfile, xmlfile)
        print("reading xml, pdb files")
        m1 = molecules.molecule(MolNum(1))[0]
        print("Virtual sites of molecule:")
        print(m1.property("virtual-sites"))
    else:
        (molecules, space) = amber.readCrdTop(crdfile.val, topfile.val)
        Sire.Stream.save((molecules, space), s3file.val)
        print("reading crd, top files")
